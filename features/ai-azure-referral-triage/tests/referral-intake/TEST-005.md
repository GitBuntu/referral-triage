# Test TEST-005: Empty Document Rejection
...
## Summary
Verify that a referral document with zero bytes (empty content) is rejected at validation, no ReferralId is assigned, no Blob Storage write occurs, and no ReferralReceived event is emitted.

## Primary Requirement
REQ-001: Referral Intake Validation and Storage

## Related Scenarios
SCENARIO-005.feature: Reject Empty Referral Document
...
2. **HTTP Response Body**
   - Assert: Response JSON contains "error" field with value "Document is empty"
   - Assert: Response JSON contains "details" field with text including "Minimum file size: 1 byte"
