# Test TEST-002: Valid Text Referral Document Acceptance
...
## Summary
Verify that a valid plain-text referral document submitted via HTTP POST is accepted, validated, stored in Blob Storage with the assigned ReferralId, and triggers a ReferralReceived domain event.

## Primary Requirement
REQ-001: Referral Intake Validation and Storage

## Related Scenarios
SCENARIO-002.feature: Accept Valid Text Referral Document
...
2. **Invariant: ReferralDocument must be preserved in original form for audit, separate from TriageRecord.**
   - After acceptance: ReferralDocument with unmodified text content is persisted in Blob Storage
...
1. **Event: ReferralReceived**
   ...
   - Payload must contain: ReferralId (UUID), DocumentId (UUID), FileType (TEXT), UploadTimestamp (RFC3339 datetime), UploadSource (string = "/referrals/intake")
...
3. **Blob Storage Persistence**
   ...
   - Assert: Stored file text content is identical to submitted text (character-for-character comparison)
   - Assert: File encoding (UTF-8) is preserved

4. **Referral Aggregate State**
   - Assert: Referral aggregate exists with ReferralId matching HTTP response
   - Assert: Referral.ReferralDocument.FileType equals "TEXT"
