# Test TEST-003: Invalid File Type Rejection
...
## Summary
Verify that a referral document in an unsupported file type (e.g., Word .docx) is rejected at validation, no ReferralId is assigned, no Blob Storage write occurs, and no ReferralReceived event is emitted.

## Primary Requirement
REQ-001: Referral Intake Validation and Storage

## Related Scenarios
SCENARIO-003.feature: Reject Invalid File Type in Referral Intake
...
1. **HTTP Response Status**
   - Assert: HTTP response status code equals 400 (Bad Request)

2. **HTTP Response Body**
   - Assert: Response JSON contains "error" field with value "Unsupported file type"
   - Assert: Response JSON contains "details" field with text including "Accepted types: PDF, plain text (.txt), scanned images (JPEG, PNG)"
